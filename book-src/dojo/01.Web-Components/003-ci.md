...
1. Vytvorte súbor `${WAC_ROOT}/ambulance-list/src/global/app.ts` a do neho umiestnite importy problematických modulov a exportujte prázdnu funkciu:
   ...
   V súbore `${WAC_ROOT}/ambulance-list/src/components/pfx-ambulance-wl-list/pfx-ambulance-wl-list.tsx` odsraňte príslušne riadky s importom `@material/web/...` modulov:
   ...
   Následne upravte súbor `${WAC_ROOT}/ambulance-list/stencil.config.ts` tak, aby obsahoval  referenciu na tento globálny skript:
...
2. Upravíme jednotkový test v súbore `${WAC_ROOT}/ambulance-list/src/components/<pfx>-ambulance-wl-list/test/<pfx>-ambulance-wl-list.spec.tsx`:
...
3. Overíme, že naša testovacia sada je funkčná a prechádza bez chýb vykonaním príkazu v priečinku `${WAC_ROOT}/ambulance-list`:
...
3. V prvej časti súboru vidíte názov predpisu, ktorý je v tomto prípade `Node.js CI`. Tento názov môžete zmeniť na `Ambulance UFE CI`, čo Vám pomôže lepšie identifikovať zdroj problémov v prípade upozornení pri zlyhaní priebežnej integrácie.
   ...
   Ďalej nasleduje sada spúšťačov _workflow_-u. V danom stave určujú, že kontinuálna integrácia bude vykonaná pri komitoch do vetvy main, ako aj pri vytvorení `pull-request` žiadostí na tejto vetve. To zabezpečí, že vetva main bude vždy obsahovať overenú verziu aplikácie. V ďalšej kapitole si ukážeme aj spúšťače pri vytváraní `release`-ov. Podrobný opis možných spúšťačov nájdete v [dokumentácii](https://docs.github.com/en/actions/reference/events-that-trigger-workflows).
