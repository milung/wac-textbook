...
V predchádzajúcom príklade sme si predstavili jeden zo spôsobov nasadenia našej aplikácie, pri ktorom sme využili služby typu _Platform as a Service_ v dátovom centre Microsoft Azure.
Hoci je tento spôsob funkčný, je vhodný len pre veľmi malé projekty. Na projektoch, kde spolupracuje niekoľko tímov a kde finálne dielo pozostáva z rôznych subsystémov, je tento spôsob nedostatočný.
...
1. V priečinku vašich lokálnych Git repozitárov (pravdepodobne na rovnakej úrovni ako je umiestnený priečinok `ambulance-list`) vytvorte nový priečinok s názvom `webcloud-gitops`. Vo VS Code pridajte tento priečinok do aktuálneho pracovného priestoru pomocou voľby menu _File->Add Folder to Workspace..._

2. V priečinku `.../webcloud-gitops` vytvorte nasledujúcu adresárovú štruktúru:
...
3. V priečinku `.../webcloud-gitops/apps/<pfx>-ambulance-ufe` vytvorte súbor `deployment.yaml` s nasledujúcim obsahom (komentáre môžete vynechať):
    ...
    spec:
      ...
      template:                    # šablóna pre vytvorenie replík podov
        ...
        spec:
          containers:
          - name: <pfx>-ambulance-ufe-container # meno kontaineru - v pod-e ich môže byť viac
            image: <your-account>/ambulance-ufe
    ...
    Súbor `deployment.yaml` je deklaráciou nasadenia našej služby - tzv. [_workload_](https://kubernetes.io/docs/concepts/workloads/) - do klastra. Všimnite si, že požaduje nasadenie dvoch replík - čo znamená, že v klastri budú vytvorené dva _pod_-y a v rámci každého z nich bude vytvorený aj bežiaci kontajner s našou aplikáciou.

    Takto vytvorený manifest je funkčný a možno ho nasadiť do klastra. Predpokladáme, že máte nainštalovaný systém kubernetes na svojom počítači a zvolený aktívny kontext pre tento klaster. To, ktorý kontext je aktívny môžete zistiť vykonaním príkazu
    ...
    *         aks            aks-cluster       user-admin
    ...
    Teraz prejdite do priečinka `.../webcloud-gitops` a vykonajte príkaz
    ...
    počítače pripojené k virtuálnej sieti kubernetes klastra. Momentálne sú tieto pody dostupné
    len na virtuálnej sieti systému kubernetes. Pokiaľ k nim chceme pristúpiť, môžeme využiť funkciu
    [presmerovania portov](https://kubernetes.io/docs/tasks/access-application-cluster/port-forward-access-application-cluster/) z lokálneho systému na cieľový pod (prípadne _service_) aktívneho klastra.
    ...
    >info:> V príkladoch uvádzame prácu s `kubectl` na príkazovom riadku. Odporúčame nainštalovať si a naučiť sa pracovať aj s aplikáciou [Kubernetes Lens][lens], ktorá poskytuje grafické rozhranie, v ktorom možno prehľadne sledovať jednotlivé zdroje systému kubernetes. Najmä v neskorších častiach cvičenia je tento nástroj užitočný pri riešení rôznych problémov.

4. Teraz vytvorte súbor `.../webcloud-gitops/apps/<pfx>-ambulance-ufe/service.yaml` s obsahom:
    ...
    spec:   
      ...
      >warning:> Meno elementu `<pfx>-ambulance-wl-list` musí zodpovedať komponentu, ktorý sme vytvorili predtým, súbor: `...\ambulance-list\src\components\<pfx>-ambulance-wl-list\<pfx>-ambulance-wl-list.tsx`

      Tento súbor je neštandardným zdrojom systému kubernetes. V ďalšom bode budeme vytvárať manifesty pre mikro-front-end _controller_, ktorý tieto zdroje obhospodáruje. V zásade ide len o registráciu mikro aplikácie, integrovanej do hlavnej aplikačnej obálky.

      Nakoniec vytvoríme súbor `.../webcloud-gitops/apps/<pfx>-ambulance-ufe/kustomization.yaml`. Správa konfigurácií založená na systéme [Kustomize] využíva súbory `kustomization.yaml` ako integračný bod jednotlivých zdrojov deklarovaných v ostatných manifestoch. Pritom ostatné manifesty ostávajú stále použiteľné a možno ich priamo použiť ako argument aplikácie `kubectl`. Súbor `kustomization.yaml` je v zásade predpisom ako automatizovane _editovať_ originálne súbory pre nasadenie jednotlivých zdrojov v iných prostrediach - rozumej v iných klastroch - a tieto úpravy možno hierarchicky kumulovať. V tomto prípade ide len o vymenovanie jednotlivých manifestov a priradenie spoločných značiek.
